...
#' @return Sampled fit.
#' @export
sample_tmb <- function(fit, nsample = 1000, fixed_only = TRUE, random_only = FALSE, both = FALSE, verbose = TRUE) {
  to_tape <- TMB:::isNullPointer(fit$obj$env$ADFun$ptr)
  if (to_tape) 
    fit$obj$retape(FALSE)
  par.full <- fit$obj$env$last.par.best  
  if(both) {
		hess <- TMB::sdreport(fit$obj, fit$fit$par, getJointPrecision=TRUE, skip.delta.method = TRUE)
		prec <- as(solve(hess), "sparseMatrix")
    smp <- rmvnorm_sparseprec(nsample, par.full, prec)
	} else if(fixed_only) {
    hess <- TMB::sdreport(fit$obj, fit$fit$par, skip.delta.method = TRUE)$cov.fixed
		prec <- as(solve(hess), "sparseMatrix")
    smp <- rmvnorm_sparseprec(nsample, par.full, prec)
  } else {
