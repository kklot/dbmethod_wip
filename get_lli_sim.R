                   ...
                   K = 10, S = 1000, verbose = F, check = F)
{
    ...
    # update data to true AFS, only cover the non-interval censor version, easy to extend
    obj$env$data$afs = fitdt$afs
    
    # how to treat age effect?
    #
    # set age effect to zero
    ...
    if (!is.null(ref_age) && ref_age == 0) # consider age_eff = 0 is no bias
      ...
      smp[, age_eff] = smp[, age_eff[which(15:49 == ref_age)]]
    # else keep age effect as is, and evaluate the true AFS with the age effect as explanatory 
    # variable instead of biases

		llk = apply(smp, 1, function(p) obj$report(p)$logliki)
    ll  = abind(ll, llk, along = 3) 
	} # end a sample
	cat("\n")
	ll
}
