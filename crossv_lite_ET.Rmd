...
di <- filter(dtab, iso == "ET" & sex == "female")
```
...
r5 = uga5 %>% as_tibble() %>%
  select(-log_yob_rw2_e, -log_age_rw2_e) %>% 
  rename_with(~paste0("y_", yob_u), yob_rw2:V23) %>% 
  rename_with(~paste0("a_", age_u), age_rw2:V53) %>% 
  mutate(smp = 1:n()) %>% 
  pivot_longer(starts_with('y_'), names_to = c(NA, 'yob'), values_to = "yobc", names_sep = "_", names_transform = list(yob = as.integer)) %>%
  pivot_longer(starts_with('a_'), names_to = c(NA, 'age'), values_to = "agec", names_sep = "_", names_transform = list(age = as.integer)) %>% 
  mutate(
...
t5 = uga5k %>% as_tibble() %>%
  select(-log_yob_rw2_e, -log_age_rw2_e) %>% 
  rename_with(~paste0("y_", yob_u), yob_rw2:V22) %>% 
  rename_with(~paste0("a_", age_u), age_rw2:V52) %>% 
  mutate(smp = 1:n()) %>% 
  pivot_longer(starts_with('y_'), names_to = c(NA, 'yob'), values_to = "yobc", names_sep = "_", names_transform = list(yob = as.integer)) %>%
  pivot_longer(starts_with('a_'), names_to = c(NA, 'age'), values_to = "agec", names_sep = "_", names_transform = list(age = as.integer)) %>% 
  mutate(
...
di %>% 
  filter(
    ...
    yob %in% plot_years) %>% 
  mutate(afs = as.numeric(afs)) %>% 
  filter(yob %in% c(1970)) %>% 
  ggplot() +
  geom_histogram(aes(afs,..density..), alpha=.5, fill="grey50", color="grey") +
  # geom_density(aes(afs), alpha=.35, fill="grey50", color="grey40") +
  geom_line(aes(age, den, color=model), v5 %>% filter(yob %in% c(1970)), size=1.2) +
  facet_wrap(~yob) +
	labs(
	  x = "Age at first sex", y = "", color="Model",
	  title = "Data and predicted age at first sex - Ethiopia",
		subtitle = "among uncensored respondents") +
  scale_fill_viridis_d() +
  scale_color_viridis_d(end=.71) +
	theme_bw() +
  guides(fill='none') +
  theme(
    strip.background = element_rect(fill='grey95', color = 'grey70', size = .3),
...
quartz_off(g2, "fig/predicted_Guinea_density", 7, 6,,1)
```