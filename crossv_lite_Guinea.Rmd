...
p5 <- uga5k %>% as_tibble(.name_repair = "universal") %>%
...
afsby %>% as_tibble() %>% 
  ...
  geom_ribbon(aes(yob, median, ymin=ci_l, ymax=ci_u, fill = factor(model)), 
              data = p45 %>% filter(model==2), alpha=.3) +
  geom_line(aes(yob, median, color = factor(model)), 
            data = p45 %>% filter(model==2)) +
