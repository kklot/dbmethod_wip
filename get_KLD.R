                   ...
                   K = 10, S = 1000, verbose = F, check = F)
{
    ...
    ages  = seq(1, 100, 1)
    # the true parameters
		scalei = tibble(yob = sort(unique(fitdt$yob))) %>% left_join(ref, 'yob')  %>% pull(scale)
		n_yob = length(scalei)
    the_P  = sapply(ages, dskewlogis, scale = scalei, shape = unique(ref$shape), skew = unique(ref$skew))
		# the estimated ones
    the_KLD = sapply(1:S, 
                function(s) {
                  the_Q = sapply(ages, dskewlogis, scale = scale[s,], shape = shape[s], skew = skew[s])
                  sapply(1:n_yob, function(y) KLD(the_P[y,], the_Q[y,])) 
                }) 
    EKLD = abind(EKLD, the_KLD, along = 3) 
	} # end a sample
	cat("\n")
	EKLD
}
